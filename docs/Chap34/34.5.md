...
The problem is in $\text{NP}$: a certificate is an injective map $f : V(G_1) \to V(G_2)$. We verify in polynomial time that whenever $(u,v) \in E(G_1)$, we also have $(f(u),f(v)) \in E(G_2)$. Then $f(G_1)$ is a subgraph of $G_2$ isomorphic to $G_1$.

To show $\text{NP}$-hardness, reduce from $\text{CLIQUE}$. Given $(G,k)$, let $G_1 = K_k$ and let $G_2 = G$. Then $G$ has a clique of size $k$ iff $K_k$ is isomorphic to a subgraph of $G$. The construction is polynomial, so subgraph isomorphism is $\text{NP}$-complete.
...
Use the standard dynamic program. Let $dp[i,s]$ be true iff some subset of the first $i$ numbers has sum $s$, for $0 \le s \le t$.

Initialize $dp[0,0]=\text{true}$ and $dp[0,s]=\text{false}$ for $s>0$. Then
$$
dp[i,s] = dp[i-1,s] \;\vee\; (s \ge a_i \text{ and } dp[i-1,s-a_i]).
$$
At the end, accept iff $dp[n,t]$ is true.

This runs in $O(nt)$ time. Since $t$ is given in unary, $t$ is polynomial in the input length, so the algorithm is polynomial-time.
...
The problem is in $\text{NP}$: a certificate is one side of the partition, and we can sum both parts in polynomial time.

For $\text{NP}$-hardness, reduce from $\text{SUBSET-SUM}$. Given an instance $(S,t)$, let $\sigma = \sum_{x \in S} x$, and assume $0 < t \le \sigma$ (otherwise the answer is trivial). Form
$$
S' = S \cup \{\,2\sigma - t,\ \sigma + t\,\}.
$$
The total sum of $S'$ is $4\sigma$, so an equal partition must split into two sets of sum $2\sigma$.

If some subset $X \subseteq S$ has sum $t$, then $X \cup \{2\sigma - t\}$ has sum $2\sigma$, giving a partition of $S'$ into equal halves.

Conversely, suppose $S'$ has an equal partition. A side of sum $2\sigma$ cannot omit both of the new numbers, since then its sum is at most $\sigma$, and it cannot contain both, since then its sum exceeds $2\sigma$. So it contains exactly one of them. If it contains $2\sigma - t$, then the old elements on that side sum to $t$. If it contains $\sigma + t$, then the old elements on the other side sum to $t$. Thus $(S,t)$ is a yes-instance of $\text{SUBSET-SUM}$.

Therefore set partition is $\text{NP}$-complete.

## 34.5-6

> Show that the hamiltonian-path problem is $\text{NP-complete}$.

The problem is in $\text{NP}$: a certificate is an ordering of the vertices, and we can check in polynomial time that consecutive vertices are adjacent and that every vertex appears exactly once.

To show $\text{NP}$-hardness, reduce from $\text{HAM-CYCLE}$. Given a graph $G$, choose a vertex $v$. Construct $G'$ by replacing $v$ with two vertices $v'$ and $v''$, each adjacent to all former neighbors of $v$, and add two new leaves $s$ and $t$ with edges $(s,v')$ and $(v'',t)$.

If $G$ has a Hamiltonian cycle, break it at $v$. The two neighbors of $v$ on the cycle give a Hamiltonian path through the old vertices, and this becomes a Hamiltonian path in $G'$ from $s$ to $t$.

Conversely, any Hamiltonian path in $G'$ must start at $s$ and end at $t$, since these are the only degree-$1$ vertices. Deleting $s$ and $t$ and identifying $v'$ and $v''$ back into one vertex $v$ yields a Hamiltonian cycle in $G$.

Hence HAM-PATH is $\text{NP}$-complete.
...
$$
\text{LONG-CYCLE} = \{\langle G,k\rangle : G \text{ has a simple cycle of length at least } k\}.
$$

This problem is in $\text{NP}$: a certificate is the list of vertices on the cycle, which can be checked in polynomial time.

It is also $\text{NP}$-hard, since
$$
G \in \text{HAM-CYCLE}
\iff
\langle G, |V(G)| \rangle \in \text{LONG-CYCLE}.
$$
Thus HAM-CYCLE reduces to LONG-CYCLE in polynomial time, so LONG-CYCLE is $\text{NP}$-complete.
...
The problem is in $\text{NP}$: given an assignment, we evaluate all clauses and count how many are true.

For $\text{NP}$-hardness, reduce from $\text{3-CNF-SAT}$. Let $\phi$ have $m$ clauses. Introduce new variables $p,q,r$, and build
$$
\phi' = \phi
\wedge \bigwedge_{i=1}^m (p \vee \neg p \vee q)
\wedge \bigwedge_{i=1}^{2m} (p \vee q \vee r).
$$
Then $\phi'$ has $4m$ clauses, an even number.

If $\phi$ is satisfiable, satisfy all clauses of $\phi$ and set $p=q=r=0$. Then the $m$ tautological clauses are true, the $2m$ clauses $(p \vee q \vee r)$ are false, and all $m$ clauses of $\phi$ are true. Hence exactly $2m$ of the $4m$ clauses are true.

Conversely, suppose $\phi'$ has an assignment with exactly half its clauses true. If any of $p,q,r$ were $1$, then all $2m$ clauses $(p \vee q \vee r)$ would be true, and the $m$ tautological clauses would also be true, giving at least $3m$ true clauses, impossible. So $p=q=r=0$. Then the $2m$ added clauses of the form $(p \vee q \vee r)$ are all false and the $m$ tautologies are all true, so to reach exactly $2m$ true clauses, all $m$ clauses of $\phi$ must be true. Thus $\phi$ is satisfiable.

Therefore half $\text{3-CNF}$ satisfiability is $\text{NP}$-complete.
