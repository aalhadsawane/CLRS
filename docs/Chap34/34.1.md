...
Let the constant number of calls be $c$, each subroutine $i$ running in time $p_i(n)$, and let the additional work take time $q(n)$, all polynomials in the input length $n$. The total running time is $\sum_{i=1}^{c} p_i(n) + q(n)$, which is a sum of a constant number of polynomials and hence itself a polynomial.

For the second part, consider a subroutine that takes a string $x$ and outputs $xx$ (i.e., $x$ concatenated with itself). This runs in $O(|x|)$ time, which is polynomial in its input size. Starting with a string of length $1$, calling this subroutine $n$ times produces a string of length $2^n$, and the total work performed is $1 + 2 + 4 + \cdots + 2^{n-1} = 2^n - 1$, which is exponential in $n$.
...
Let $A_1$ and $A_2$ be the polynomial-time algorithms deciding $L_1$ and $L_2$ in times $p_1(n)$ and $p_2(n)$.

**Complement**: On input $x$, run $A_1$ and accept iff it rejects. Total time $O(p_1(n))$.

**Union**: On input $x$, run $A_1$ and $A_2$; accept iff either accepts. Total time $O(p_1(n) + p_2(n))$.

**Intersection**: On input $x$, run $A_1$ and $A_2$; accept iff both accept. Total time $O(p_1(n) + p_2(n))$.

**Concatenation**: On input $x$ of length $n$, try all $n + 1$ ways to split $x = yz$ and accept iff $A_1$ accepts $y$ and $A_2$ accepts $z$ for some split. There are $O(n)$ splits each taking $O(p_1(n) + p_2(n))$, giving polynomial total time.

**Kleene star**: On input $x$ of length $n$, let $dp[i] = 1$ if $x[1..i] \in L_1^\*$. Set $dp[0] = 1$. For $i = 1$ to $n$, set $dp[i] = 1$ if there exists $j < i$ with $dp[j] = 1$ and $A_1$ accepts $x[j+1..i]$. Accept iff $dp[n] = 1$. This uses $O(n^2)$ calls to $A_1$ on inputs of length at most $n$, so the total time $O(n^2 \cdot p_1(n))$ is polynomial.
