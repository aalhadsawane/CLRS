...
Reconnect the two path pieces in the noncrossing way (equivalently, replace by one of the two pairings that preserves a single Hamiltonian cycle). For crossing segments in the plane,
$$
\|a-b\|+\|c-d\|>\|a-c\|+\|b-d\|
$$
(strict triangle inequality via the intersection point).
